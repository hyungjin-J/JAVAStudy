...
public class MapPractice {
	 ...
	 * - 키 : 중복 허용하지 않음(Set 방식)
	 * - 밸류 : 키 값이 중복되지 않는 경우 중복 허용 (List 방식)
	 */
	...
	while(true) {
		...
		switch(menu) {
		case 1: // 회원 가입 : 아이디 비밀번호 입력 받아 Map에 추가
		...
		case 2:
			// 로그인 => 아이디 비밀번호를 입력받아서, Map에 아이디(키값)가 있는지 확인
			// 있으면 비밀번호를 비교 --> 없으면 "아이디가 존재하지 않습니다."
			// Map 에 아이디에 해당하는 비밀번호와 입력된 비밀번호가 일치하는지 확인
			...
			if (map.containsKey(id2)) {
				...
				if (map.get(id2).equals(pwd2)) {
					//입력한 비밀번호와 저장되어 있는 비밀번호가 일치하는 경우
		...
		case 3:
			...
			//Map 에 존재하지 않는 경우 "아이디가 존재하지 않습니다."
		...
		case 4:
			// 회원 탈퇴
			// 아이디, 비밀번호를 입력 받아 모두 일치하는 경우 탈퇴(삭제)
